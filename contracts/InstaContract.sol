...
contract InstaContract {
    ...
    mapping(string => address) userMap;
    ...
    function getUser(
        ...
        activeOnly
        hasAcc
        returns (address, string memory, address)
    {
    ...
    ) external activeOnly hasAcc returns (uint) {
    ...
    function getFollowers(
        address addr
    ) external view activeOnly hasAcc returns (address[] memory) {
        address a = accounts[addr];
        require(a != address(0), "no account");
        InstaAccount acc = InstaAccount(a);
