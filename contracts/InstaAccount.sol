...
contract InstaAccount {
    ...
    modifier hasPost(uint id) {
        require(
            bytes(posts[id].tag).length != 0 ||
                bytes(posts[id].hash).length != 0,
            "UNAUTHORIZED"
        );
        _;
    }
    ...
    function post(
        string memory tag,
        string memory hash
    ) public adminCheck returns (uint) {
        p_count += 1;
    ...
    function toggleLike(address addr, uint id) public adminCheck hasPost(id) {
        ...
        for (uint x = 0; x < likes.length; x++) {
            ...
            if (flag) likes[x - 1] = posts[id].likes[x];
        }
        if (flag) likes.length--;
        else likes.push(addr);
    }
    ...
    function remfollower(address addr) public adminCheck {
        ...
        for (uint x = 0; x < followers.length; x++) {
            ...
            if (flag) followers[x - 1] = followers[x];
        }
        if (flag) followers.length--;
    }
    ...
    function remfollowing(address addr) public adminCheck {
        ...
        for (uint x = 0; x < following.length; x++) {
            ...
            if (flag) following[x - 1] = following[x];
        }
        following.length--;
    }
